...
from ensim.core.types import CalculationError
...
class TestH2O2Combustion:
    ...
    def test_stoichiometric_combustion_temperature(self, species_db):
        ...
        result = problem.solve(
            pressure=1013250.0, initial_temp_guess=3000.0, max_iterations=100, tolerance=1e-5
        )
    ...
    def test_h2o_is_major_product(self, species_db):
        ...
        assert x_h2o > 0.5, f"H2O mole fraction = {x_h2o:.4f}, expected > 0.5"
    ...
    def test_mass_conservation(self, species_db):
        ...
        assert rel_error < 1e-8, f"Mass conservation error: {rel_error * 100:.6f}%"
