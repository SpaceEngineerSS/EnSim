...
from ensim.core.propulsion import calculate_c_star, calculate_thrust_coefficient
from ensim.utils.nasa_parser import create_sample_database, load_default_database
...
class CEATestCase:
    ...
    # NASA CEA 3.3.2 HP reference values for gas-phase reactants at 298.15 K
    cea_T_chamber: float  # K
    cea_gamma: float  # frozen Cp/Cv, not equilibrium speed-of-sound gamma
    cea_mean_mw: float  # g/mol
    # Tolerances
    T_tolerance: float = 0.02  # 2%


# Generated with NASA's official ``cea==3.3.2`` Python package. The reactant
# mole basis, phase, inlet temperature, pressure, and equilibrium constraint
# are explicit so every reference value can be reproduced.
CEA_TEST_CASES = [
    CEATestCase(
        name="H2(g)/O2(g), stoichiometric",
        fuel="H2",
        oxidizer="O2",
        of_ratio=7.9367,
        fuel_moles=2.0,
        ...
        cea_T_chamber=3674.145461891216,
        cea_gamma=1.1934338358757215,
        cea_mean_mw=15.767676142439509,
    ),
    CEATestCase(
        name="CH4(g)/O2(g), stoichiometric",
        fuel="CH4",
        oxidizer="O2",
        of_ratio=3.9893,
        fuel_moles=1.0,
        ...
        cea_T_chamber=3680.1661433072672,
        cea_gamma=1.196241036975546,
        cea_mean_mw=22.837038689894722,
    ),
]
...
class TestNASACEAComparison:
    ...
    @pytest.fixture
    def species_db(self):
        """Load the same packaged database used by the application."""
    ...
    def test_chamber_temperature(self, test_case: CEATestCase, species_db):
        ...
        """
        assert test_case.fuel in species_db
        assert test_case.oxidizer in species_db
        ...
        result = problem.solve(
            pressure=test_case.chamber_pressure, initial_temp_guess=3000.0, max_iterations=100
        )
...
class TestThrustCoefficient:
    ...
    def _calculate_pressure_ratio(self, gamma: float, area_ratio: float) -> float:
        ...
        from ensim.core.propulsion import solve_mach_from_area_ratio_supersonic
