...
from ensim.core.instability import (
    InstabilityResult,
    ModeType,
...
class TestFullAnalysis:
    ...
    def test_basic_analysis(self):
        ...
        result = analyze_combustion_instability(
            chamber_length=0.3, chamber_diameter=0.2, gamma=1.2, T_chamber=3500.0, mean_mw=18.0
        )
        ...
        assert result.chugging_frequency is None
        assert result.is_stable is None
        assert result.assessment_level == "acoustic_modes_only"
    ...
    def test_stability_requires_modal_rate_data(self):
        """No verdict is manufactured without modal response data."""
        result = analyze_combustion_instability(
            chamber_length=0.3,
            ...
            T_chamber=3500.0,
            mean_mw=18.0,
            modal_rate_data={"1L": (30.0, 45.0), "1T": (50.0, 40.0)},
        )

        assert len(result.stability_margins) == 2
        assert result.is_stable is False
        assert result.assessment_level == "modal_growth_rate_balance"
    ...
    def test_lumped_chug_frequency_uses_inertance_and_compliance(self):
        frequency = calculate_chugging_frequency(
            propellant_density=1000.0,
            feed_line_length=2.0,
            feed_line_area=0.002,
            system_compliance=1e-11,
        )
        expected = 1.0 / (2.0 * np.pi) * np.sqrt(0.002 / (1000.0 * 2.0 * 1e-11))
        assert frequency == pytest.approx(expected)

    def test_partial_feed_model_is_rejected(self):
...
class TestQuickCheck:
    ...
    def test_returns_string(self):
        ...
        summary = quick_stability_check(chamber_length=0.3, chamber_diameter=0.2)
...
class TestNASASP194Examples:
    ...
    def test_f1_engine_like_chamber(self):
        """Test a representative full-scale chamber geometry."""
        D = 0.91  # ~36 inches
        L = 0.58  # ~23 inches
        ...
        assert 200 < f_1t < 1000, f"1T = {f_1t:.0f} Hz outside the geometry-scale interval"
