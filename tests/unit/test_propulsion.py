...
class TestThrustCoefficient:
    ...
    def test_cf_vacuum_vs_sea_level(self):
        ...
        assert Cf_vacuum > Cf_sea, f"Vacuum Cf = {Cf_vacuum:.3f}, Sea Level Cf = {Cf_sea:.3f}"
...
class TestPerformanceIntegration:
    ...
    def test_h2o2_rocket_performance(self):
        ...
        assert 400 < result.isp < 500, f"Isp = {result.isp:.1f} s, expected 400-500 s"
        assert 1500 < result.c_star < 2500, f"C* = {result.c_star:.1f} m/s, expected 1500-2500 m/s"
        assert 3500 < result.exit_velocity < 5000, (
            f"Ve = {result.exit_velocity:.1f} m/s, expected 3500-5000 m/s"
        )
        assert result.exit_mach > 3.0, f"M_e = {result.exit_mach:.2f}, expected > 3"
    ...
    def test_sea_level_performance(self):
        ...
        nozzle_vac = NozzleConditions(area_ratio=50.0, chamber_pressure=P_c, ambient_pressure=0.0)
    ...
    def test_thrust_mass_flow_and_isp_remain_consistent_with_efficiencies(self):
        pressure = 6.0e6
        result = calculate_performance(
            ...
            1.21,
            21.0,
            NozzleConditions(25.0, pressure, 20_000.0, 0.004),
            eta_cstar=0.94,
            eta_cf=0.96,
            alpha_deg=18.0,
        )

        assert result.mass_flow_rate is not None
        assert result.thrust == pytest.approx(result.mass_flow_rate * 9.80665 * result.isp)

    def test_mismatched_chamber_pressures_are_rejected(self):
        with pytest.raises(ValueError, match="pressures must agree"):
            calculate_performance(
                3400.0,
                6.0e6,
                1.21,
                21.0,
                NozzleConditions(25.0, 5.0e6, 0.0),
            )
...
class TestSuttonExample:
    ...
    def test_c_star_sutton(self, sutton_conditions):
        ...
        c_star = calculate_c_star(sutton_conditions["gamma"], R_specific, sutton_conditions["T_c"])
