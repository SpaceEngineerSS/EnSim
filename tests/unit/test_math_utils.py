...
class TestQuaternionMultiplication:
    ...
    def test_q_mult_non_commutative(self):
        ...
        q1 = q_from_axis_angle(np.array([1.0, 0.0, 0.0]), np.pi / 4)
        q2 = q_from_axis_angle(np.array([0.0, 1.0, 0.0]), np.pi / 4)
    ...
    def test_q_mult_associative(self):
        ...
        q1 = q_from_axis_angle(np.array([1.0, 0.0, 0.0]), np.pi / 6)
        q2 = q_from_axis_angle(np.array([0.0, 1.0, 0.0]), np.pi / 4)
        q3 = q_from_axis_angle(np.array([0.0, 0.0, 1.0]), np.pi / 3)
...
class TestVectorRotation:
    ...
    def test_rotate_90deg_x(self):
        ...
        q = q_from_axis_angle(np.array([1.0, 0.0, 0.0]), np.pi / 2)
        v_rot = q_rotate_vector(q, v)
    ...
    def test_rotate_90deg_y(self):
        ...
        q = q_from_axis_angle(np.array([0.0, 1.0, 0.0]), np.pi / 2)
        v_rot = q_rotate_vector(q, v)
        expected = np.array([0.0, 0.0, -1.0])
        assert_allclose(v_rot, expected, atol=1e-10)
    ...
    def test_rotate_90deg_z(self):
        ...
        v = np.array([1.0, 0.0, 0.0])
        q = q_from_axis_angle(np.array([0.0, 0.0, 1.0]), np.pi / 2)
        v_rot = q_rotate_vector(q, v)
    ...
    def test_rotate_180deg(self):
        ...
        v = np.array([1.0, 0.0, 0.0])
        q = q_from_axis_angle(np.array([0.0, 0.0, 1.0]), np.pi)
        v_rot = q_rotate_vector(q, v)
    ...
    def test_rotate_preserves_magnitude(self):
        ...
        v = np.array([1.0, 2.0, 3.0])
        q = q_from_axis_angle(np.array([1.0, 1.0, 1.0]), np.pi / 3)
        v_rot = q_rotate_vector(q, v)
...
class TestAxisAngleConversion:
    ...
    def test_axis_angle_identity(self):
        ...
        q = q_from_axis_angle(np.array([1.0, 0.0, 0.0]), 0.0)
        assert_allclose(q, q_identity(), atol=1e-10)
...
class TestRotationMatrix:
    ...
    @pytest.mark.parametrize(
        "angles",
        [(0.0, 0.0, 0.0), (0.3, -0.7, 1.2), (np.pi, 0.0, 0.0)],
    )
    def test_rotation_matrix_round_trip(self, angles):
        original = q_from_euler(*angles)
        recovered = q_from_rotation_matrix(q_to_rotation_matrix(original))
        assert_allclose(
            q_to_rotation_matrix(recovered),
            q_to_rotation_matrix(original),
            atol=1e-12,
        )

    def test_rotation_matrix_input_validation(self):
        with pytest.raises(ValueError, match="orthonormal"):
            q_from_rotation_matrix(np.diag([1.0, 1.0, 2.0]))
