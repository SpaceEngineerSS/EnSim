...
from ensim.core.thermodynamics import (
    CRITICAL_PROPERTIES,
...
class TestVirialCompressibility:
    ...
    def test_ideal_gas_at_low_pressure(self):
        ...
        assert pytest.approx(1.0, abs=0.1) == Z
...
class TestRedlichKwongCompressibility:
    ...
    def test_ideal_gas_at_low_pressure(self):
        ...
        assert pytest.approx(1.0, abs=0.05) == Z
    ...
    def test_physically_reasonable_range(self):
        ...
        Z = calculate_compressibility_factor_rk(T=200.0, P=20e6, Tc=150.0, Pc=5e6)
        assert 0.1 < Z < 3.0
...
class TestMixtureCompressibility:
    ...
    def test_pure_species_h2(self):
        ...
        Z = get_mixture_compressibility(T=300.0, P=1e6, composition={"H2": 1.0}, method="virial")
        # H2 is nearly ideal at moderate conditions
        assert 0.9 < Z < 1.1

    def test_pure_species_h2o(self):
        """Pure H2O compressibility."""
        Z = get_mixture_compressibility(T=500.0, P=1e6, composition={"H2O": 1.0}, method="virial")
        assert 0.8 < Z < 1.2

    def test_mixture_combustion_products(self):
        """Typical combustion products mixture."""
        composition = {"H2O": 0.6, "CO2": 0.2, "N2": 0.15, "H2": 0.05}
    ...
    def test_empty_composition_is_rejected(self):
        with pytest.raises(ValueError):
            get_mixture_compressibility(T=300.0, P=1e6, composition={}, method="virial")

    def test_unknown_species_is_rejected(self):
        with pytest.raises(KeyError):
            get_mixture_compressibility(
                T=300.0, P=1e6, composition={"UnknownSpecies": 1.0}, method="virial"
            )
...
class TestDensityCorrection:
    ...
    def test_invalid_z_is_rejected(self):
        with pytest.raises(ValueError):
            correct_density_for_real_gas(1.5, Z=0.0)
...
class TestEnthalpyDeparture:
    ...
    def test_high_pressure_larger_departure(self):
        ...
        dH_low = correct_enthalpy_departure(T=300.0, P=1e5, Tc=150.0, Pc=5e6, omega=0.0)
        dH_high = correct_enthalpy_departure(T=300.0, P=5e6, Tc=150.0, Pc=5e6, omega=0.0)
        assert abs(dH_high) > abs(dH_low)

    def test_invalid_critical_properties_are_rejected(self):
        with pytest.raises(ValueError):
            correct_enthalpy_departure(
                T=300.0,
                P=1e6,
