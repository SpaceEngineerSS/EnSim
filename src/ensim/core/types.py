...
class SpeciesData:
    ...
    coeffs_high: NDArray[np.float64] = field(default_factory=lambda: np.zeros(7, dtype=np.float64))
    coeffs_low: NDArray[np.float64] = field(default_factory=lambda: np.zeros(7, dtype=np.float64))
...
class ThermoResult:
    ...
    def from_dimensionless(
        cls, T: float, P: float, cp_r: float, h_rt: float, s_r: float, R: float = 8.31446261815324
    ) -> "ThermoResult":
...
class EquilibriumResult:
    ...
    Attributes:
        ...
        element_balance_error: Maximum scaled elemental conservation residual
        enthalpy_error: Product enthalpy minus reactant enthalpy (J)
    """
    ...
    converged: bool = True
    iterations: int = 0
    element_balance_error: float = 0.0
    enthalpy_error: float = 0.0
