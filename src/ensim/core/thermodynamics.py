...
def h_over_rt(T: float, coeffs: NDArray[np.float64]) -> float:
    ...
    return a1 + (a2 / 2.0) * T + (a3 / 3.0) * T**2 + (a4 / 4.0) * T**3 + (a5 / 5.0) * T**4 + a6 / T
...
def s_over_r(T: float, coeffs: NDArray[np.float64]) -> float:
    ...
    return a1 * np.log(T) + a2 * T + (a3 / 2.0) * T**2 + (a4 / 3.0) * T**3 + (a5 / 4.0) * T**4 + a7
...
def calculate_compressibility_factor_virial(
    T: float, P: float, Tc: float, Pc: float, omega: float = 0.0
) -> float:
    """
    ...
    Uses the Pitzer-Curl correlation for the second virial coefficient B.
    ...
    Reference:
        Pitzer, K.S. and Curl, R.F. (1957). "The Volumetric and
        Thermodynamic Properties of Fluids. III. Empirical Equation for the
        Second Virial Coefficient." J. Am. Chem. Soc. 79(10): 2369-2370.
    """
    if T <= 0 or P < 0 or Tc <= 0 or Pc <= 0:
        raise ValueError("T, Tc and Pc must be positive and P must be non-negative")
    if P == 0:
        return 1.0
    ...
    if Z <= 0.0:
        raise ValueError("the truncated virial model produced non-positive compressibility")
    return Z


def calculate_compressibility_factor_rk(T: float, P: float, Tc: float, Pc: float) -> float:
    """
    ...
    admissible = real_roots[real_roots > B]
    if admissible.size == 0:
        raise ValueError("Redlich-Kwong equation has no admissible real root")
    return float(np.max(admissible))
...
def get_mixture_compressibility(
    T: float, P: float, composition: dict[str, float], method: str = "virial"
) -> float:
    """
    ...
    if not composition or any(value < 0.0 for value in composition.values()):
        raise ValueError("composition must contain non-negative mole fractions")
    if not np.isclose(composition_total, 1.0, rtol=0.0, atol=1e-8):
        raise ValueError("composition mole fractions must sum to one")
    if not np.isclose(total_fraction, 1.0, rtol=0.0, atol=1e-8):
        missing = sorted(set(composition) - set(CRITICAL_PROPERTIES))
        raise KeyError(f"critical properties are unavailable for: {', '.join(missing)}")
    ...
    raise ValueError("method must be 'virial' or 'rk'")


@jit(nopython=True, cache=True)
def correct_density_for_real_gas(rho_ideal: float, Z: float) -> float:
    """
    ...
    """
    if rho_ideal < 0.0 or Z <= 0.0:
        raise ValueError("rho_ideal must be non-negative and Z must be positive")
    return rho_ideal / Z
...
def correct_enthalpy_departure(
    ...
    Uses the pressure derivative implied by the truncated Pitzer-Curl
