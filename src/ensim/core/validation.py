...
def validate_expansion_ratio(
    expansion_ratio: float, ambient_pressure_bar: float = 0.0
    ...
    if at_sea_level and expansion_ratio > EXPANSION_RATIO_MAX_SEA_LEVEL:
        issues.append(
            ValidationIssue(
                ...
                field="Expansion Ratio",
                message=(
                    f"High ε={expansion_ratio:.0f} at sea level; attached-flow performance "
                    "may not represent the internal nozzle flow"
                ),
                value=expansion_ratio,
...
def validate_efficiency(eta: float, efficiency_type: str = "C*") -> list[ValidationIssue]:
    """
