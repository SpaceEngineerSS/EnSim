...
class NozzleConditions:
    ...
    def __repr__(self) -> str:
        return (
            f"NozzleConditions(ε={self.area_ratio:.1f}, "
...
class PerformanceResult:
    ...
    def __repr__(self) -> str:
        return (
            f"PerformanceResult(Isp={self.isp:.1f} s, "
...
def calculate_throat_conditions(
    gamma: float, T_chamber: float, P_chamber: float
) -> tuple[float, float]:
    """
...
def solve_mach_from_area_ratio_supersonic(
    area_ratio: float, gamma: float, tol: float = 1e-8, max_iter: int = 50
) -> float:
    """
...
def calculate_exit_conditions(
    gamma: float, T_chamber: float, P_chamber: float, exit_mach: float
...
def calculate_performance(
    ...
    """
    Calculate frozen ideal-nozzle performance with explicit efficiencies.
    ...
    Applied factors:
        ...
        - Cf_adjusted = η_Cf × (λ Cf_momentum + Cf_pressure)
    ...
    scalars = (
        ...
        nozzle.area_ratio,
        nozzle.chamber_pressure,
        nozzle.ambient_pressure,
        eta_cstar,
        eta_cf,
        alpha_deg,
    )
    if not all(np.isfinite(value) for value in scalars):
        raise ValueError("Nozzle-performance inputs must be finite")
    if T_chamber <= 0.0 or P_chamber <= 0.0 or mean_molecular_weight <= 0.0:
        raise ValueError("Temperature, chamber pressure, and molecular weight must be positive")
    if not 1.0 < gamma < 2.0:
        raise ValueError("gamma must be between one and two")
    if nozzle.area_ratio < 1.0 or nozzle.ambient_pressure < 0.0:
        raise ValueError("Area ratio must be at least one and ambient pressure cannot be negative")
    if not np.isclose(nozzle.chamber_pressure, P_chamber, rtol=1e-12):
        raise ValueError("Nozzle and performance chamber pressures must agree")
    if nozzle.throat_area is not None and (
        not np.isfinite(nozzle.throat_area) or nozzle.throat_area <= 0.0
    ):
        raise ValueError("Throat area must be positive and finite")
    if not 0.0 < eta_cstar <= 1.0 or not 0.0 < eta_cf <= 1.0:
        raise ValueError("Efficiency factors must be in the interval (0, 1]")
    if not 0.0 <= alpha_deg < 90.0:
        raise ValueError("Nozzle half-angle must be in the interval [0, 90) degrees")
    critical_back_pressure = P_chamber * (2.0 / (gamma + 1.0)) ** (gamma / (gamma - 1.0))
    if nozzle.ambient_pressure > critical_back_pressure:
        raise ValueError("Ambient pressure is too high for the assumed choked nozzle flow")
    ...
    cf_momentum = calculate_thrust_coefficient(
        gamma, pressure_ratio, nozzle.area_ratio, pressure_ratio
    )
    cf_pressure = nozzle.area_ratio * (pressure_ratio - ambient_ratio)
    ...
    Cf = eta_cf * (divergence_factor * cf_momentum + cf_pressure)
    ...
    if nozzle.throat_area is not None:
        ...
        thrust = Cf * P_chamber * nozzle.throat_area
    else:
        mass_flow = None
...
def get_nozzle_profile(
    ...
    n_points: int = 100,
    area_ratios: np.ndarray | None = None,
) -> dict:
    """
    ...
    Args:
        ...
        R_specific: Specific gas constant (J/kg/K)
        n_points: Number of points when area_ratios is not supplied
        area_ratios: Optional local A/A* values for a supplied contour
    ...
    if T_chamber <= 0.0 or P_chamber <= 0.0 or R_specific <= 0.0:
        raise ValueError("Chamber state and specific gas constant must be positive")
    if exit_area_ratio <= 1.0:
        raise ValueError("Exit area ratio must be greater than one")
    supplied_stations = area_ratios is not None
    if area_ratios is None:
        if n_points < 2:
            raise ValueError("At least two nozzle-profile stations are required")
        area_ratios = np.linspace(1.0, exit_area_ratio, n_points)
    else:
        area_ratios = np.asarray(area_ratios, dtype=float)
        if area_ratios.ndim != 1 or len(area_ratios) < 2:
            raise ValueError("area_ratios must be a one-dimensional station array")
        if not np.all(np.isfinite(area_ratios)) or np.any(area_ratios < 1.0):
            raise ValueError("All local area ratios must be finite and at least one")

    mach, temp, pressure, _ = _compute_profile_arrays(gamma, T_chamber, P_chamber, area_ratios)
    ...
    if supplied_stations:
        x_norm = np.linspace(0.0, 1.0, len(area_ratios))
