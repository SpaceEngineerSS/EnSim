...
def q_rotate_vector(q: np.ndarray, v: np.ndarray) -> np.ndarray:
    ...
    # v' = v + 2w(ω × v) + 2(ω × (ω × v))
    return np.array(
        [v[0] + 2.0 * (w * cx + ccx), v[1] + 2.0 * (w * cy + ccy), v[2] + 2.0 * (w * cz + ccz)]
    )
...
def q_from_rotation_matrix(matrix: np.ndarray) -> np.ndarray:
    """Convert a proper 3D rotation matrix to a unit quaternion."""
    matrix = np.asarray(matrix, dtype=np.float64)
    if matrix.shape != (3, 3):
        raise ValueError("Rotation matrix must have shape (3, 3)")
    if not np.all(np.isfinite(matrix)):
        raise ValueError("Rotation matrix must contain finite values")
