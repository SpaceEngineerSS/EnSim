...
class SnapshotManager:
    ...
    _snapshots: list[DesignSnapshot]
    _max_snapshots: int
...
class CompareWindow(QDialog):
    ...
    def _update_table(self, snap1: DesignSnapshot | None, snap2: DesignSnapshot | None):
    ...
    def _update_plot(self, snap1: DesignSnapshot | None, snap2: DesignSnapshot | None):
        ...
        if not snap1 and not snap2:
            ax.text(0.5, 0.5, "Select designs to compare", ha="center", va="center", color="#666")
            self.canvas.draw()
            return
        ...
        if snap1:
            vals1 = [snap1.isp_vacuum, snap1.thrust_kn, snap1.c_star / 100, snap1.temperature / 100]
        if snap2:
            vals2 = [snap2.isp_vacuum, snap2.thrust_kn, snap2.c_star / 100, snap2.temperature / 100]
        ...
        if vals1:
            ax.bar(
                ...
                vals1,
                width,
                label=snap1.name if snap1 else "A",
                color="#00a8ff",
                alpha=0.8,
            )
        if vals2:
            ax.bar(
                ...
                vals2,
                width,
                label=snap2.name if snap2 else "B",
                color="#ff6b6b",
                alpha=0.8,
            )
