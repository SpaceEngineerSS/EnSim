...
class PerformanceGraph(QWidget):
    ...
    def update_plots(
        ...
        self.ax_pressure.semilogy(
            profile["area_ratio"], profile["P_ratio"], color="#66fcf1", linewidth=2, label="P/Pc"
        )
        self.ax_pressure.set_ylabel("P/Pc", color="#c5c6c7")
        self.ax_pressure.set_title(
            "PRESSURE PROFILE", fontsize=10, fontweight="bold", color="#66fcf1"
        )
        self.ax_pressure.axhline(y=1.0, color="#ff9f1c", linestyle="--", linewidth=1)
        self.ax_pressure.fill_between(
            profile["area_ratio"], profile["P_ratio"], alpha=0.2, color="#66fcf1"
        )

        # Temperature plot
        self.ax_temp.plot(
            profile["area_ratio"], profile["temperature"], color="#ff9f1c", linewidth=2, label="T"
        )
        self.ax_temp.set_ylabel("Temperature (K)", color="#c5c6c7")
        self.ax_temp.set_title(
            "TEMPERATURE PROFILE", fontsize=10, fontweight="bold", color="#66fcf1"
        )
        self.ax_temp.axhline(
            y=T_chamber, color="#45a29e", linestyle="--", linewidth=1, label=f"Tc={T_chamber:.0f}K"
        )
        self.ax_temp.fill_between(
            profile["area_ratio"], profile["temperature"], alpha=0.2, color="#ff9f1c"
        )

        # Mach number plot
        self.ax_mach.plot(
            profile["area_ratio"], profile["mach"], color="#66fcf1", linewidth=2, label="Mach"
        )
        self.ax_mach.set_ylabel("Mach Number", color="#c5c6c7")
        self.ax_mach.set_xlabel("Area Ratio (A/A*)", color="#c5c6c7")
        self.ax_mach.set_title(
            "MACH NUMBER PROFILE", fontsize=10, fontweight="bold", color="#66fcf1"
        )
        self.ax_mach.axhline(
            y=1.0, color="#ffffff", linestyle="--", linewidth=1, label="Sonic (M=1)"
        )
        self.ax_mach.fill_between(
            profile["area_ratio"], profile["mach"], alpha=0.2, color="#66fcf1"
        )
        ...
        for ax in [self.ax_pressure, self.ax_temp, self.ax_mach]:
            ...
            ax.legend(
                loc="upper right", facecolor="#0b0c10", edgecolor="#45a29e", labelcolor="#c5c6c7"
            )
    ...
    def _save_plot_png(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Save Plot", "nozzle_profile.png", "PNG Images (*.png);;All Files (*)"
        )
        if path:
            self.figure.savefig(
                path, dpi=300, facecolor="#1e1e1e", edgecolor="none", bbox_inches="tight"
            )
    ...
    def _save_plot_svg(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Save Plot", "nozzle_profile.svg", "SVG Images (*.svg);;All Files (*)"
        )
        if path:
            self.figure.savefig(
                path, format="svg", facecolor="#1e1e1e", edgecolor="none", bbox_inches="tight"
            )
