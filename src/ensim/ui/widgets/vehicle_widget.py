...
class VehicleDesignerWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.axial_cd_spin = self._create_spinbox(0.01, 3.0, 0.45, decimals=3, step=0.01)
        self.axial_cd_spin.setToolTip("Axial drag coefficient referenced to body frontal area")
        body_card.add_row("Axial Cd:", self.axial_cd_spin)
        ...
        self.fin_span_spin = self._create_spinbox(0.02, 0.2, 0.12, "m", step=0.01)
        ...
        recovery_card.form.addRow(self.descent_rate_label)
    ...
    def _update_rocket(self):
        ...
        from ensim.core.rocket import NoseShape
        ...
        # Update nose
        self._rocket.nose.shape = shape_map.get(
            self.nose_shape_combo.currentIndex(), NoseShape.OGIVE
        )
        ...
        self._rocket.axial_drag_coefficient = self.axial_cd_spin.value()
    ...
    def _update_diagram(self):
        """Update the rocket side-view diagram."""
        if self._rocket is None:
            return

        from ensim.core.aero import analyze_rocket
        ...
        nose_poly = Polygon(
            list(zip(nose_x, nose_y_top, strict=False))
            + list(zip(nose_x[::-1], nose_y_bot[::-1], strict=False)),
            closed=True,
            facecolor="#00d4ff",
            edgecolor="#ffffff",
            linewidth=2,
        ...
        ax.annotate(
            "CG", (cg, R * 1.8), color="#00ff9d", fontsize=11, ha="center", fontweight="bold"
        )
        ...
        ax.annotate(
            "CP", (cp, -R * 2.0), color="#ff3366", fontsize=11, ha="center", fontweight="bold"
        )
        ...
        # Scale reference
        ax.plot([0, 0.5], [-R - fin_span - 0.08, -R - fin_span - 0.08], "w-", linewidth=2)
        ax.text(0.25, -R - fin_span - 0.12, "0.5 m", color="#8899aa", ha="center", fontsize=10)
        ...
        if margin_cal >= 1.0:
            ...
            status = f"UNSTABLE ({margin_cal:.2f} cal)"
            color = "#ff3366"
    ...
    def _update_recovery_estimate(self):
        ...
        try:
            from ensim.core.recovery import Parachute

            chute = Parachute(diameter=self.chute_diameter_spin.value())
            mass = self._rocket.dry_mass
            v_descent = chute.get_descent_rate(mass)
            self.descent_rate_label.setText(f"Estimated descent: {v_descent:.1f} m/s")
    ...
    def update_flight_plots(self, flight_result):
        """Update the flight result plots."""
        ...
        ax1.plot(
            flight_result.time,
            flight_result.altitude / 1000,
            color=plot_style["line_colors"][0],
            linewidth=2,
        )
        ax1.axhline(
            y=flight_result.apogee_altitude / 1000,
            color="#ff6b35",
            linestyle="--",
            alpha=0.8,
        ...
        ax2.plot(
            flight_result.range / 1000,
            flight_result.altitude / 1000,
            color=plot_style["line_colors"][1],
            linewidth=2,
        )
        ...
        ax3.plot(
            flight_result.time,
            flight_result.stability_margin,
            color=plot_style["line_colors"][2],
            linewidth=2,
        )
        ...
        ax4.plot(
            flight_result.time,
            flight_result.q / 1000,
            color=plot_style["line_colors"][3],
            linewidth=2,
        )
    ...
    def update_flight_plots_6dof(self, flight_result):
        ...
        ax1.plot(
            flight_result.time,
            flight_result.position_z,
            color=plot_style["line_colors"][0],
            linewidth=2,
        )
        ax1.axhline(
            y=flight_result.apogee_altitude,
            color="#ff6b35",
            linestyle="--",
            alpha=0.8,
        ...
        ax4 = self.flight_figure.add_subplot(gs[1, 1])
        velocity = np.sqrt(
            flight_result.velocity_x**2 + flight_result.velocity_y**2 + flight_result.velocity_z**2
        )
        ax4.plot(flight_result.time, velocity, color=plot_style["line_colors"][3], linewidth=2)
        ax4.axhline(y=flight_result.max_velocity, color="#00d4ff", linestyle="--", alpha=0.6)
        ax4.set_xlabel("Time (s)", color=plot_style["text_color"])
