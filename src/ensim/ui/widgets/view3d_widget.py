...
from ensim.core.constants import GAS_CONSTANT
...
HEADLESS_QT = os.environ.get("QT_QPA_PLATFORM", "").lower() in {"offscreen", "minimal"}
RENDERING_DISABLED = os.environ.get("ENSIM_DISABLE_3D", "").lower() in {"1", "true", "yes"}
PYVISTA_AVAILABLE = PYVISTA_IMPORTABLE and not HEADLESS_QT and not RENDERING_DISABLED
...
class NozzleView3D(QWidget):
    ...
    def _setup_ui(self):
        ...
        if PYVISTA_AVAILABLE:
            ...
            if RENDERING_DISABLED:
                message = "3D visualization is disabled for this session"
            elif PYVISTA_IMPORTABLE:
                message = "3D visualization is unavailable on this display platform"
            else:
                message = "3D visualization requires PyVista"
            ...
            if RENDERING_DISABLED:
                detail = "Unset ENSIM_DISABLE_3D to enable rendering"
            elif PYVISTA_IMPORTABLE:
                detail = "Use a desktop session with OpenGL 3.2 or later"
            else:
                detail = "Install the optional 3D visualization dependencies"
            install_label = QLabel(detail)
    ...
    def update_view(
        ...
        profile = get_nozzle_profile(
            ...
            area_ratios=contour.area_ratio,
        )
        ...
        if mesh is not None:
            ...
            # Add info text
            self._plotter.add_text(
                f"Area ratio {exit_area_ratio:.1f}  |  Tc {T_chamber:.0f} K",
                position="upper_left",
                font_size=10,
                color="#888888",
                name="info",
    ...
    def _create_nozzle_mesh(self, x: np.ndarray, r: np.ndarray, profile: dict, color_by: str):
        """
        ...
        try:
            ...
            if len(profile["temperature"]) != n_x:
                ...
                temp_interp = interp1d(
                    x_profile, profile["temperature"], kind="linear", fill_value="extrapolate"
                )
                mach_interp = interp1d(
                    x_profile, profile["mach"], kind="linear", fill_value="extrapolate"
                )
            ...
            if color_by not in mesh.array_names:
                raise ValueError(f"Unknown nozzle color field: {color_by}")
            mesh.set_active_scalars(color_by)

            return mesh

        except Exception:
            logger.exception("Could not create the nozzle mesh")
            return None
    ...
    def export_stl(self, filepath: str, binary: bool = True) -> bool:
        ...
        if not PYVISTA_AVAILABLE:
            logger.error("PyVista is unavailable for STL export")
            return False

        if self._mesh is None:
            logger.error("No nozzle mesh is available for export")
            return False
        ...
        try:
            ...
            logger.info("Exported STL mesh to %s", filepath)
            return True

        except Exception:
            logger.exception("Could not export STL mesh to %s", filepath)
            return False
    ...
    def export_ply(self, filepath: str) -> bool:
        ...
        try:
            ...
            logger.info("Exported PLY mesh to %s", filepath)
            return True

        except Exception:
            logger.exception("Could not export PLY mesh to %s", filepath)
            return False
    ...
    def export_obj(self, filepath: str) -> bool:
        ...
        try:
            ...
            logger.info("Exported OBJ mesh to %s", filepath)
            return True

        except Exception:
            logger.exception("Could not export OBJ mesh to %s", filepath)
            return False
    ...
    def update_attitude(self, q: np.ndarray):
        ...
        try:
            ...
            self._plotter.add_mesh(
                transformed_mesh, color="#00d4ff", show_edges=False, smooth_shading=True
            )

        except Exception:
            logger.exception("Could not update the rendered attitude")
