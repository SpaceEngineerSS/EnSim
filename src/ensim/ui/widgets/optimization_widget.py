...
class NozzleOptimizationTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        note = QLabel(
            "Ideal frozen, calorically perfect nozzle model. Boundary-layer losses, "
            "two-phase flow, and flow separation are not assessed."
        )
...
class StageMassOptimizationTab(QWidget):
    ...
    def _update_isp_inputs(self):
        ...
        for widget in self.structure_inputs:
            widget.deleteLater()
        self.structure_inputs.clear()
        ...
        for i in range(n):
            ...
            structure = QDoubleSpinBox()
            structure.setRange(0.01, 0.50)
            structure.setValue(0.10)
            structure.setDecimals(3)
            structure.setToolTip("Dry mass / (dry mass + propellant mass)")
            self.isp_layout.addWidget(QLabel("Structural coefficient:"), i, 2)
            self.isp_layout.addWidget(structure, i, 3)
            self.structure_inputs.append(structure)
    ...
    def _run_optimization(self):
        ...
        params = {
            ...
            "stage_isps": isps,
            "structural_coefficients": [spin.value() for spin in self.structure_inputs],
        }
    ...
    def _on_finished(self, result: OptimizationResult):
        ...
        if result.success:
            ...
            for i, (mass, frac) in enumerate(zip(masses, fractions, strict=False)):
...
class EngineOptimizationTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        range_layout.addWidget(QLabel("Expansion Ratio:"), 2, 0, Qt.AlignmentFlag.AlignRight)
        self.expansion_ratio = QDoubleSpinBox()
        self.expansion_ratio.setRange(1.1, 500.0)
        self.expansion_ratio.setValue(40.0)
        self.expansion_ratio.setDecimals(1)
        range_layout.addWidget(self.expansion_ratio, 2, 1)

        range_layout.addWidget(QLabel("Ambient Pressure:"), 2, 2, Qt.AlignmentFlag.AlignRight)
        self.ambient_pressure = QDoubleSpinBox()
        self.ambient_pressure.setRange(0.0, 0.15)
        self.ambient_pressure.setValue(0.0)
        self.ambient_pressure.setDecimals(6)
        self.ambient_pressure.setSuffix(" MPa")
        range_layout.addWidget(self.ambient_pressure, 2, 3)

        self.propellant.currentTextChanged.connect(self._update_propellant_bounds)
        ...
        layout.addWidget(results_group)
        note = QLabel(
            "Gibbs-equilibrium chamber chemistry with an ideal frozen nozzle. "
            "Throat area is sized from the requested thrust; hardware mass and cooling "
            "constraints are not part of this optimization."
        )
        note.setWordWrap(True)
        note.setObjectName("notesText")
        layout.addWidget(note)
    ...
    def _update_propellant_bounds(self, propellant: str):
        bounds = {
            "LOX/CH4": (2.5, 4.0),
            "LOX/RP1": (2.0, 3.5),
            "LOX/LH2": (4.0, 8.0),
        }
        minimum, maximum = bounds[propellant]
        self.of_min.setValue(minimum)
        self.of_max.setValue(maximum)
