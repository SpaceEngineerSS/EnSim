"""
Altitude Performance Widget - Thrust and Isp vs altitude visualization.
"""
...
class MissionAnalysisWidget(QWidget):
    """
    Widget for steady engine-performance sweeps over altitude.

    Shows thrust, Isp, and exit-to-ambient pressure ratio versus altitude.
    """
    ...
    def _setup_ui(self):
        ...
        # Mission parameters
        mission_group = QGroupBox("Altitude Sweep")
        ...
        # Analysis button
        self.analyze_btn = QPushButton("Run Altitude Sweep")
    ...
    def _create_empty_plots(self):
        ...
        self.ax_status.set_title("Exit-to-Ambient Pressure Ratio", color="white", fontweight="bold")
    ...
    def update_plots(self, mission_result):
        ...
        self.ax_thrust.scatter(
            [alt_km[max_idx]], [thrust_kN[max_idx]], color="#ffff00", s=100, zorder=5, marker="*"
        )
        ...
        # Exit-to-ambient pressure-ratio plot
        pe_pa = mission_result.pressure_ratio
        finite = np.isfinite(pe_pa)
        self.ax_status.plot(alt_km[finite], pe_pa[finite], color="#00e5a0", linewidth=2)
        self.ax_status.axhline(
            y=1.0, color="#00ff88", linestyle="--", linewidth=2, label="Matched (1.0)"
        )

        # Mark optimal expansion altitude
        if mission_result.optimal_altitude < mission_result.altitudes[-1]:
            self.ax_status.axvline(
                x=mission_result.optimal_altitude / 1000,
                color="#00ff88",
                linestyle=":",
                linewidth=2,
            )
