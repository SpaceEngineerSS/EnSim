...
def compute_thermo(
    ...
    for j in range(n_spec):
        c = coeffs_high[j] if T >= t_mid[j] else coeffs_low[j]
...
class CombustionProblem:
    ...
    def __init__(self, species_db: SpeciesDatabase):
        ...
        self.default_products = [
            # Major H/O species
            'H2O', 'H2', 'O2', 'OH', 'H', 'O', 'HO2', 'H2O2',
            # Carbon species
            'CO2', 'CO', 'CH4', 'C2H2', 'C2H4', 'CH2O', 'CHO',
            # Nitrogen species (for RP-1, UDMH, etc.)
            'N2', 'NO', 'NO2', 'N', 'N2O', 'NH3', 'HCN', 'CN',
            # Additional radicals for high-temperature accuracy
            'HNO', 'NH', 'NH2'
        ]
    
    def add_fuel(self, species_name: str, moles: float = 1.0,
                 temperature: float = 298.15) -> None:
        self.reactants.append(Reactant(species_name, moles, temperature))
    
    def add_oxidizer(self, species_name: str, moles: float = 1.0,
                     temperature: float = 298.15) -> None:
        self.reactants.append(Reactant(species_name, moles, temperature))
    ...
    def calculate_input_enthalpy(self) -> float:
        ...
        for r in self.reactants:
            if r.species_name in self.species_db:
                ...
                T = r.temperature
                c = sp.coeffs_low if T < sp.t_mid else sp.coeffs_high
                h_rt = c[0] + c[1]/2*T + c[2]/3*T**2 + c[3]/4*T**3 + c[4]/5*T**4 + c[5]/T
