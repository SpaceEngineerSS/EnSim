...
from src.core.math_utils import (
    q_mult, q_conjugate, q_normalize, q_rotate_vector,
    q_from_euler, q_to_euler, q_derivative, q_identity,
    cross_product, dot_product, vector_norm, normalize_vector
)
from src.core.integrators import (
    DP5_C, DP5_A, DP5_B5, DP5_E,
    rk45_error_norm, compute_new_step_size,
    cubic_hermite_interpolate, linear_interpolate
)
...
def simulate_flight_6dof(
    ...
    for i in range(n_steps):
        ...
        if has_propellant:
            ...
            if throttled_mdot > 0:
                isp_current = F_thrust_mag / (throttled_mdot * G0)
            else:
                isp_current = isp_vac
