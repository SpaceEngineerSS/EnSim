...
from src.core.rocket import BodyTube, FinSet, NoseCone, NoseShape, Rocket
...
def calculate_cp_fins(fins: FinSet, body_length_to_fins: float, body_diameter: float) -> float:
    ...
    x_fin_local = Lf * (Cr + 2 * Ct) / (3 * (Cr + Ct)) + (Cr + Ct - Cr * Ct / (Cr + Ct)) / 6
...
def calculate_drag_coefficient(rocket: Rocket, mach: float, reynolds: float = 1e7) -> float:
    """
    ...
    # Wetted area ratio
    A_wet = (
        rocket.nose.volume * 0.5 / rocket.nose.length  # Nose approximation
        + rocket.body.wetted_area
        + rocket.fins.total_area * 2
    )  # Both sides of fins
